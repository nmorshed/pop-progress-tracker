(function () {
  ...
  function usePlanEditor(props) {
    ...
    function save(complete) { if (!editable) return Promise.resolve(false); setSaving(true); props.setError(''); return api('action-plans/' + props.plan.id, { method: 'PUT', body: { title: props.plan.title, revision: props.plan.revision, data: props.plan.data, completeWeek: !!complete } }).then(function (saved) { props.updatePlan(saved); setDirty(false); props.setNotice(complete ? 'Week submitted successfully.' : 'All changes saved.'); return true; }).catch(function (err) { props.setError(message(err)); return false; }).finally(function () { setSaving(false); }); }
